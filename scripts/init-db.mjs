...
async function main() {
    ...
    try {
        ...
        dbName = (pathPart ? decodeURIComponent(pathPart) : '').replace(/["']+$/g, '').replace(/^["']+/g, '')
        // Fallback: nombre desde variable de entorno (útil en despliegue cuando la URL no incluye /nombre_db)
        if (!dbName && process.env.DATABASE_NAME) {
            dbName = process.env.DATABASE_NAME.replace(/^["'\s]+|["'\s]+$/g, '')
        }
        if (!dbName) {
            console.error('❌ No se encontró nombre de base de datos en DATABASE_URL.')
            console.error('   En despliegue: define DATABASE_URL con el path, ej. mysql://user:pass@host:3306/nombre_db')
            console.error('   O define la variable DATABASE_NAME con el nombre de la base de datos.')
            process.exit(1)
        }
        connectionParams = {
