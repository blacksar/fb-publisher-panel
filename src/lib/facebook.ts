...
// Helper: Convertir ruta local (/uploads/...) o URL remota (Coolify) a Base64 real para enviar a FB
async function getRealBase64(imagePathOrBase64: string): Promise<string> {
    ...
    if (imagePathOrBase64.startsWith("http://") || imagePathOrBase64.startsWith("https://")) {
        try {
            const res = await fetch(imagePathOrBase64, { cache: "no-store" })
            ...
            console.error("Error descargando imagen remota (Coolify):", e)
...
export async function publishToFacebook(
    ...
    let realBase64: string | undefined = postData.imageBase64
    if (realBase64) {
        realBase64 = await getRealBase64(realBase64)
        if (realBase64 && !realBase64.startsWith("data:image/")) {
            console.warn("[publishToFacebook] imagen no es base64 válido (¿archivo /uploads/ no existe en este servidor?):", realBase64.slice(0, 50))
            realBase64 = undefined
        }
    }
