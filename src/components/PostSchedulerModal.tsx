...
export function PostSchedulerModal({
  ...
  // Establecer hora por defecto: siempre 1h después del último post programado de la página
  const prevOpenAndPageRef = useRef({ open: false, pageId: "", hadLast: false })
  useEffect(() => {
    const last = lastScheduledForPage
    const wasOpen = prevOpenAndPageRef.current.open
    const prevPageId = prevOpenAndPageRef.current.pageId
    const prevHadLast = prevOpenAndPageRef.current.hadLast
    // Siempre actualizar la ref (también al cerrar) para que la próxima apertura detecte didOpen
    ...
    const didOpen = !wasOpen && open
    const didChangePage = prevPageId !== formData.page_id
    const lastJustLoaded = !prevHadLast && !!last?.scheduled_at
  ...
  const handleCommentPaste = (e: React.ClipboardEvent<HTMLTextAreaElement>) => {
    ...
    const newComment = comment.slice(0, start) + pasted + comment.slice(end)
    e.preventDefault()
  ...
  const handleContentPaste = (e: React.ClipboardEvent<HTMLTextAreaElement>) => {
    ...
    const newContent = content.slice(0, start) + pasted + content.slice(end)
    e.preventDefault()
